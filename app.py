"""
Pawdcast Skit Factory — FINAL WORKING VERSION (December 2025)
Audio Mode: 100% perfect sync with Google TTS / Gemini / any audio
"""
...
def split_audio_file(audio_path, split_times, total_duration, output_dir):
    ...
    for i in range(len(times) - 1):
        ...
        output_files.append({"path": output_path, "start": start, "duration": duration})
...
# ============================================================================
# MAIN APP — ONLY FIXED AUDIO MODE
# ============================================================================
def main():
    ...
    mode = st.radio("Mode", ["audio", "skit", "article"], horizontal=True,
                    format_func=lambda x: {"audio":"Audio Mode (FREE & PERFECT)", "skit":"Skit Mode", "article":"Article Mode"}[x])
    ...
    if mode == "audio":
        ...
        if st.button("Create Perfect Video", use_container_width=True):
            if not all([audio_file, skit_text, templates_ready]):
                st.error("Missing files or skit")
            else:
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    try:
                        ...
                        st.info(f"Split points: 0, {', '.join([f'{t:.2f}' for t in split_times])}")
    ...
    # skit & article modes unchanged (you can keep your original code here)
